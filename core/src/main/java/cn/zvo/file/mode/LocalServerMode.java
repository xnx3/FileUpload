...
public class LocalServerMode implements StorageModeInterface{
	...
	public void putStringFile(String path, String text, String encode) {
		directoryInit(path);
		try {
			FileUtil.write(this.localFilePath+path, text, encode);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
//	}
	...
	public UploadFileVO put(String path, InputStream inputStream) {
		...
		directoryInit(path);
		File file = new File(this.localFilePath+path);
		OutputStream os;
		try {
	...
	public String getTextByPath(String path) {
		String text = FileUtil.read(this.localFilePath+path, FileUtil.UTF8);
	...
	public void deleteObject(String filePath) {
		FileUtil.deleteFile(this.localFilePath+filePath);
	}
...
//	}
	...
	public long getDirectorySize(String path) {
		directoryInit(path);
		return sizeOfDirectory(new File(this.localFilePath+path));
	}

	@Override
	public void copyObject(String originalFilePath, String newFilePath) {
		directoryInit(newFilePath);
		FileUtil.copyFile(this.localFilePath + originalFilePath, this.localFilePath + newFilePath);
	}
		...
		if(!FileUtil.exists(path)){
			...
			for (int i = 0; i < ps.length; i++) {
				if(ps[i].length() > 0){
					...
					if(!FileUtil.exists(this.localFilePath+xiangdui)){
						File file = new File(this.localFilePath+xiangdui);
						file.mkdir();
					}
				}
			}
